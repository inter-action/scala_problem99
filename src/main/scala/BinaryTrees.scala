...
object binarytree{
  ...
  case class Node[+T](value: T, left: Tree[T], right: Tree[T]) extends Tree[T] {
     ...
     */
    def layoutBinaryTree2 = {
      var maxY = 0
      // max_depth of Tree
      // child_left_node_x = parent_node_x - 2**(max_depth-child_depth)
      // child_left_node_y = parent_node_y + 1
      // mapping.update(child_left, (child_left_x, child_left_y)
      // do same as child_right_node
      // find root node x index: root_node_x
      // iterate all nodes in map add the offset: root_node_x
      // rebuild tree. @layoutBinaryTree.traverse
      Tree.inorderTravers(this)( (node: Node[T], h) =>{
        if (h > maxY){
          maxY = h
        }
      ...
      val mapping = collection.mutable.Map[T, (T, Int, Int)]()
      var x:Int = 1
      Tree.inorderTravers(this)((node: Node[T], h)=>{
      })
    }
